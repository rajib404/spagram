...
import { sendBookingRejectedToClient } from "@/lib/email";
  ...
  try {
    ...
    if (token) {
      ...
      if (session.user.role !== "THERAPIST" && session.user.role !== "ADMIN") {
        return NextResponse.json(
          { error: "Only therapists can decline bookings" },
          { status: 403 }
        );
      }
      ...
      if (
        ...
        return NextResponse.json(
          { error: "You can only decline your own bookings" },
          { status: 403 }
        );
      }
    }

    // Wrap status check + Stripe cancel + DB update in transaction
    ...
    // Send declined email to client (non-blocking)
    const emailData = buildBookingEmailData(updatedBooking);
    sendBookingRejectedToClient(emailData).catch((err) =>
    ...
    logger.error("Reject booking error", { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json(
